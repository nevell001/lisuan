...
public class DataManager {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(INVENTORY_FILE))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (parts.length >= 12) {
                    ...
                    product.spec = spec;
                    product.minStock = minStock;
                    product.cost = price * 0.7; // 默认成本价为售价的70%
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(PROMOTIONS_FILE))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (parts.length >= 10) {
                    ...
                    promotion.usageCount = 0; // 重新加载时使用次数重置
    ...
    public static List<Shift> loadShifts() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(SHIFTS_FILE))) {
            String line;
            while ((line = reader.readLine()) != null) {
                String[] parts = line.split("\\|", -1);  // 使用-1保留末尾的空字符串
                if (parts.length == 12) {
                    // 旧格式（没有支付方式收入）
                    ...
                    System.out.println("警告: 班次记录格式错误，字段数=" + parts.length + ", 期望=12或16, 行内容: " + line);
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
