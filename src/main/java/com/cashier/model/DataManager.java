...
public class DataManager {
    ...
    public static void restoreData(String backupPath) throws IOException {
        File backupDir = new File(backupPath);
        if (!backupDir.exists()) {
            throw new IOException("备份目录不存�? " + backupPath);
        }
    ...
    public static List<Category> loadCategories() {
        ...
        if (!file.exists()) {
            ...
            categories.add(new Category("食品", "食品类商�?));
            categories.add(new Category("饮料", "饮品类商�?));
            categories.add(new Category("日用�?, "日用品类商品"));
            return categories;
        }
    ...
    public static Map<String, User> loadUsers() {
        ...
        if (!file.exists()) {
            // 如果用户文件不存在，创建默认管理员账�?
            User admin = new User("admin", "admin123", "系统管理�?, "admin");
