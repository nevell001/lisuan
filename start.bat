...
REM ============================================
REM   LiSuan Start Script (Enhanced)
REM   Version 2.5.4
REM ============================================
...
REM Check for Java processes containing LiSuan or lisuan
tasklist /FI "IMAGENAME eq java.exe" /V 2>NUL | findstr /I "LiSuan\|lisuan" >NUL 2>&1
if not errorlevel 1 (
...
if %DOCKER_AVAILABLE%==1 (
    docker ps -f name=lisuan-mysql --format "{{.State}}" >nul 2>&1
    if errorlevel 1 (
        echo [INFO] MySQL container is not running
    ) else (
        echo [OK] MySQL container is running: lisuan-mysql
        set "MYSQL_CONTAINER_RUNNING=1"
    )
) else (
...
if not exist "%DB_CONFIG_FILE%" (
    ...
    if "!DB_TYPE!"=="1" (
        ...
        set /p "DB_NAME=Enter database name [lisuan_system]: "
        if "!DB_NAME!"=="" set "DB_NAME=lisuan_system"
